...
def makePdf(jpgFileName):
    ...
    pdfFileName=jpgFileName.replace('.jpg','.pdf')
...
def mergefiles(path, output_filename, import_bookmarks=False):
    ...
    #filelist = getfilenames(filepath=path, file_ext='.jpg')
    path = os.path.abspath('.')
    filelist=scan_files(path,prefix=None,postfix='.pdf')
    print(filelist)
    print('==========================')
...
path = os.path.abspath('.')
rarList=scan_files(path,prefix=None,postfix='.rar')
print('running...')
new_path=os.path.abspath('..')
if rarList:
    for rarName in rarList:
        tempName=rarName.replace(path,'')
        tempName=tempName.replace('\\','')
        patoolib.extract_archive(tempName, outdir=".")
        time.sleep(0.1)
        jpgList=scan_files(path,prefix=None,postfix='.jpg')
        for jpgName in jpgList:
            makePdf(jpgName)
        removeList=scan_files(path,prefix=None,postfix='.pdf')
        tempName=tempName.replace('.rar','.pdf')
        MIX(tempName)
        time.sleep(0.1)
        shutil.move(path+'\\'+tempName,new_path+'\\tempRarFile\\'+tempName)
        for jpgName in jpgList:
            os.remove(jpgName)
        count=0
        for oldPDF in removeList:
            if count == 0:
                count=1
                pass
            else:
